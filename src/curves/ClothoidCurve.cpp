...
ClothoidCurve::ClothoidCurve(Position start, double startCurvature, double curvatureRate, double length) : BaseCurve(0, length), start(start), startCurvature(startCurvature), curvatureRate(curvatureRate), length(length) {
}
...
double ClothoidCurve::getValueForLength(double ti, double length, double h) {
    return std::min(ti + length, length);
}
